...
function parseArgs(argv) {
  const args = {
    config: 'docker/container-config.json',
    out: '',
...
/**
 * Resolve secrets configuration for compose generation.
 * Supports:
 *  - file: top-level `secrets: { config_json: { file: <path> } }` and service reference
 *  - external: top-level `secrets: { <name>: { external: true } }` and service reference
 *  - none: no top-level secrets and no service secret references
 *
 * @param {{secretsMode:string,secretsFile:string,secretsExternalName:string,secretsTarget:string}} opts
 * @returns {{ topLevel:Object, serviceRef:Array }}
 */
