#!/usr/bin/env bash
#
# /mnt/shared/drive-personal/Documents/Programming/foundryvtt-over-my-head/docker/scripts/pod-handler.sh
#
# Small access point wrapper around the compose.dev.yml in the parent folder.
# Usage: pod-handler.sh <command> [service] [args...]
#
# Commands:
...
#   shell SERVICE      Open an interactive shell in a service (sh/ash)
...
# Resolve default compose file
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_COMPOSE_FILE="$SCRIPT_DIR/../compose.dev.yml"

COMPOSE_FILE="${COMPOSE_FILE:-}"
if [ -z "$COMPOSE_FILE" ]; then
  # Prefer repo-root docker/compose.dev.yml if invoked from repo root
...
# Choose docker compose invocation
...
Options:
  -f, --file <compose.yml>  Path to docker compose file (default: docker/compose.dev.yml or docker/scripts/../compose.dev.yml)
...
case "$cmd" in
  ...
  exec)
    ...
    svc="$1"; shift
    # Default command to sh for these images; run as root to match compose file
    if [ $# -eq 0 ]; then
  ...
  shell)
    ...
    svc="$1"; shift
    # Prefer sh/ash/bash whichever exists in container
