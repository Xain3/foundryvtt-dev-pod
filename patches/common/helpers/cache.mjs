			...
			while (redirects <= maxRedirects) {
				...
				const res = await new Promise((resolve, reject) => {
					const req = lib.request(currentUrl, opts, (resp) => {
						try { resp.setTimeout(timeoutMs, () => { resp.destroy(new Error(`response timeout after ${timeoutMs}ms for ${currentUrl}`)); }); } catch {}
						resolve(resp);
					});
					try { req.setTimeout(timeoutMs, () => { req.destroy(new Error(`request timeout after ${timeoutMs}ms for ${currentUrl}`)); }); } catch {}
					req.on("error", reject);
				...
				if ([301, 302, 303, 307, 308].includes(status)) {
					...
					currentUrl = nextUrl; redirects++; continue;
				}
				return res;
			}
...
export async function fetchToFileWithCache(url, cacheDir, { dryRun = false, debug = false, cacheMode = 'revalidate' } = {}, etagMeta) {
	...
	await new Promise((resolve, reject) => {
		...
		let finished = false;
		const dlTimeout = setTimeout(() => { if (!finished) { try { out.destroy(new Error(`download timeout for ${url}`)); } catch {} try { resp.destroy(new Error(`download timeout for ${url}`)); } catch {} } }, 30000);
...
export class CacheManager {
	...
	async fetchToFileWithCache(url) { const meta = this.readMetaForUrl(url); return fetchToFileWithCache(url, this.cacheDir, { dryRun: this.dryRun, debug: this.debug, cacheMode: this.cacheMode }, meta); }
	...
	async hasLocalFileChanged(filePath) {
		...
		if (!shouldChecksum) { const newMeta = { size: stat.size, mtimeMs: stat.mtimeMs }; this.writeMetaForKey(key, newMeta); return { changed: basicChanged, meta: newMeta }; }
		const sha = sha256File(filePath); const changed = !meta || meta.sha256 !== sha; const newMeta = { size: stat.size, mtimeMs: stat.mtimeMs, sha256: sha }; this.writeMetaForKey(key, newMeta); return { changed, meta: newMeta };
	}
	_listFiles(dir, base, acc, maxFiles) {
		const entries = fs.readdirSync(dir, { withFileTypes: true });
		for (const e of entries) {
			const full = path.join(dir, e.name); const rel = path.relative(base, full);
			if (e.isDirectory()) { this._listFiles(full, base, acc, maxFiles); }
			else if (e.isFile()) { const st = fs.statSync(full); acc.push({ rel, size: st.size, mtimeMs: st.mtimeMs }); if (acc.length > maxFiles) return; }
	...
	async hasLocalDirectoryChanged(dirPath) {
		...
		if (files.length > this.dirMaxFiles) { return { changed: true, reason: 'too-many-files' }; }
...
export default { CacheManager, sha256File, fetchWithRetry, fetchToFileWithCache, readMetaForUrl };
