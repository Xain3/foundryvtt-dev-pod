...
describe('patch scripts accept --procedural-number and --patch-name', () => {
  ...
  const r = runScript('patches/common/use-cache-or-stagger.mjs', ['--procedural-number', '01', '--patch-name', 'x-test'], { CONTAINER_CACHE: '/does/not/exist', FETCH_STAGGER_SECONDS: '0', PATCH_DRY_RUN: '1' });
    expect([0, null]).toContain(r.status);
    expect(r.stdout + r.stderr).toMatch(/\[patch] 01-x-test:/);
  });

  test('sync-host-content.mjs uses prefix', () => {
  const r = runScript('patches/common/sync-host-content.mjs', ['--procedural-number', '11', '--patch-name', 'y-test', '--initial-only'], { PATCH_DRY_RUN: '1', WORLD_SYNC_ENABLED: '0' });
    expect([0, null]).toContain(r.status);
